...
export class OAuthService {
    ...
    switch (grantTokenDto.scope) {
      ...
      default:
        throw new BadRequestException(
          `scope ${grantTokenDto.scope} is not supported.`,
        );
    }
  }
  ...
  async refreshToken(refreshToken: string) {
    if (
      !(await this.accessTokenRepo.findOneBy({ refresh_token: refreshToken }))
    ) {
      throw new BadRequestException(
    ...
    try {
      const { userId, scope, resourceOwner } =
  ...
  async isAuthenticateUser(email: string, password: string) {
    ...
    if (user && isValidPassword && !this.isAccessLockedUser(user)) {
      return true;
    } else {
  ...
  async loginUser(grantTokenDto: GrantTokenDto) {
    ...
    const isValidAuthenticate = await this.isAuthenticateUser(
      grantTokenDto.email,
