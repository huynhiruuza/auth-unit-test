...
interface IFileFieldOptions
  extends Omit<ApiPropertyOptions, 'type' | 'format'> {
  each?: boolean;
  ...
  if (options?.regex) {
    decorators.push(
      Matches(new RegExp(options?.regex.pattern), {
        message:
          options.regex.message || i18nValidationMessage('validation.Regex'),
      }),
    );
  }
...
export function NumberField(
  ...
  if (int) {
    decorators.push(
      IsInt({ each, message: i18nValidationMessage('validation.IsInt') }),
    );
  } else {
    decorators.push(
      IsNumber(
  ...
  if (isNumber(minimum)) {
    decorators.push(
      Min(minimum, { each, message: i18nValidationMessage('validation.Min') }),
    );
  }

  if (isNumber(maximum)) {
    decorators.push(
      Max(maximum, { each, message: i18nValidationMessage('validation.Max') }),
    );
  }
...
export function FileField(options?: IFileFieldOptions): PropertyDecorator {
  const { fileSize, fileTypes, maxCount, minCount, each, ...swaggerOptions } =
    options;
  const decorators = [
  ...
  if (fileTypes) {
    decorators.push(
      HasMimeType(fileTypes, {
        each,
        message: i18nValidationMessage('validation.HasMimeType'),
      }),
    );
  }
  ...
  if (fileSize) {
    decorators.push(
      MaxFileSize(fileSize * 1000000, {
        each,
        message: i18nValidationMessage('validation.MaxFileSize'),
      }),
    );
  }
